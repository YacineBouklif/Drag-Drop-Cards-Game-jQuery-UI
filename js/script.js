...
function init() {
  ...
  var words = [ 'Rabbit', 'Cat', 'Lamb', 'Goat', 'Pigeon', 'Cow' ];
  for ( var i=1; i<=6; i++ ) {
    $('<div class="draggable-item"><span class="image-item"></span><span class="words-item">' + words[i-1] + '</span></div>').data( 'number', i ).appendTo( '#cardSlots' ).droppable( {
      accept: '#cardPile div',
      hoverClass: 'hovered',
      drop: handleCardDrop
    } );
